...
impl BankTracker {
    ...
    /// Get the state of all banks for a pedal type (e.g., Microcosm has 16 banks: 45-60)
    pub fn get_bank_state(&self, pedal_type: &str, bank_range: std::ops::RangeInclusive<u8>) -> Result<Vec<BankSlot>> {
        ...
        for bank_num in bank_range {
            let bank_number = BankNumber::new(bank_num)?;
    ...
    pub fn assign_to_bank(&self, pedal_type: &str, bank_number: BankNumber, preset_id: &PresetId) -> Result<()> {
        ...
        // Assign to bank
        self.repository.assign_to_bank(pedal_type, bank_number.value(), preset_id)?;
        
        Ok(())
    }
    
    /// Clear a bank assignment (mark as empty)
    pub fn clear_bank(&self, pedal_type: &str, bank_number: BankNumber) -> Result<()> {
        self.repository.clear_bank(pedal_type, bank_number.value())?;
        Ok(())
    }
    
    /// Get the preset assigned to a specific bank (if any)
    pub fn get_bank_preset(&self, pedal_type: &str, bank_number: BankNumber) -> Result<Option<Preset>> {
        let assignments = self.repository.get_bank_assignments(pedal_type)?;
        
        for (bank_num, preset_id, _synced_at) in assignments {
            if bank_num == bank_number.value() {
